		    ...
		    title: '值班时间'
			    ...
			    title: '值班人员'
				  ...
				  formatter:function(value,row,index){
					  ...
					  var deleteButton = "<button onclick=\"myDeleteClick("+row.wkid+")\" type=\"button\" class=\"btn btn-link\"><span class=\"glyphicon glyphicon-trash\" style=\"color: rgb(0, 0, 60); font-size: 17px;\"> </span></button>";
...
function myDeleteClick(id) {
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/worktime/deleteWorktime.action",//根据编号查询用户的信息
		 data:{wkid:wkid},//传参数到后台
		 dataType:"json",//以json格式传递数据
		 success:function(data){
				$("#table").bootstrapTable("refresh");
			  	//给出提示信息
